...
#define CV_CDS_RED 0.5          //ColorValue_CdS_Red  
#define CV_CDS_BLUE 0.8         //ColorValue_CdS_Blue
...
{   
    ...
    //turning and moving towards ticket kiosk
    ...
    //getting light through just moving
    if (1){
    turnLeft(10.0,15.);
    moveForward(3.5,15.);
    ...
    //getting light through loop
    if (0){
    //moving until cds gets a good reading
    turnLeft(7.0,15.);
    ...
    while (i==1){
        ...
        lightValue = sensor_cds.Value();
        LCD.WriteLine(lightValue);
        if ((CV_CDS_BLUE-0.05)<=lightValue<=(CV_CDS_BLUE+0.05)){
            //moving to blue button
            LCD.WriteLine("Blue light Found");
            i=0;
        }
        else if((CV_CDS_RED-0.05)<=lightValue<=(CV_CDS_RED+0.05)){
            //moving to red button
            LCD.WriteLine("Red Light Found");
            i=0;
        }
        else if(forkbumper.Value()==BP){
            LCD.WriteLine("No light detecting within values");
            i=0;
        }
    }
    leftMotor.Stop();
